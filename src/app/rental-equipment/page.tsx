        ...
        <div className="container mx-auto px-4 md:px-8">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 md:gap-6">
            {categories.map((cat) => (
              <div
                key={cat.num}
                className="bg-white rounded-[20px] p-7 md:p-8 flex flex-col gap-5 shadow-[0_2px_16px_rgba(0,0,0,0.07)] hover:shadow-[0_8px_32px_rgba(0,0,0,0.13)] hover:-translate-y-1 transition-all duration-300"
              >
                <span className="font-mono text-[11px] uppercase tracking-[0.18em] text-brand-gray-400">
                  {cat.num}
                </span>
                ...
                <Link
                  href="/contact?inquiry=Equipment+Rental"
                  className="group inline-flex items-center gap-2 self-start rounded-full bg-brand-gray-900 text-white font-bold text-[14px] px-5 py-2.5 hover:bg-brand-red transition-colors"
                >
                  Rent
                  <ArrowUpRight className="w-3.5 h-3.5 transition-transform group-hover:rotate-45" strokeWidth={2} />
                </Link>
              </div>
            ))}
          </div>
